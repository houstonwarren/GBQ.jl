...
###################################### GENERATE DATA #######################################
function polynomial_2d_og(x, y)
    return sin(x*y) / y
end

function generate_p2d_og_data(n, lb, ub, rng)
    n_dim = length(lb)
    dim_delta = ub - lb
    ...
    z = map(i -> polynomial_2d_og(X[i, :]...), 1:size(X, 1))
    return X, z
end

X, y = generate_p2d_og_data(n_full_data, [-3.0, -3.0], [3.0, 3.0], global_rng)
data = hcat(X, y)

# plot
surface(-4:0.1:4, -2.5:0.1:2.5, polynomial_2d_og)
...
# quadrature solution
quadrature(polynomial_2d_og, [-3.0, -3.0], [3.0, 3.0])  # 0
...
function run_once(;
    ...
    ############### GENERATE DATA ################
    X_train, y_train = generate_data(generate_p2d_og_data, n_train, lb, ub, noise_sd, rng)
    ...
    ### quadrature
    # quad_est = quadrature(polynomial_2d_og, lb, ub, noise_sd, n_train).u
    quad_est = 1e-2

    ### mc integration
    mc_est, mc_sd = mc_quadrature(polynomial_2d_og, lb, ub, n_train, noise_sd, rng)

    # quasi mc integration
    qmc_est, qmc_sd = mc_quadrature_with_data(polynomial_2d_og, lb, ub, X_train, noise_sd, rng)

    ## bq
    pₓ = MvNormal(μₓ, diagm(Σₓ))
    bq_est = bayesian_quadrature(X_train, diagm(ls).^2, pₓ, noisy_K_rff, y_train, lb, ub)
...
# save
# save_results(mean_df, std_df, err_df, runs_per_n, "experiments/2D/p2d_og.hdf5")
CSV.write("experiments/2D/results/p2d_og_means.csv", mean_df)
CSV.write("experiments/2D/results/p2d_og_stds.csv", std_df)
CSV.write("experiments/2D/results/p2d_og_err_means.csv", err_df)
CSV.write("experiments/2D/results/p2d_og_err_stds.csv", err_σ_df)
CSV.write("experiments/2D/results/p2d_og_data.csv", DataFrame(data, ["x1", "x2", "y"]))
