...
###################################### GENERATE DATA #######################################
function disjoint_poly_1d(x)
    if x < 2.5
        out = (x-4)^2 * .2*x^3 - 3x -3
    else
        out = (x-4)^2 * .2*x^3 - 3x -13
    end
    return out
end

function generate_disjoint1d_data(n, lb, ub, rng)
    n_dim = length(lb)
    dim_delta = ub - lb
    halton_samples = halton_sampler(n, n_dim, rng)
    deltas = halton_samples .* dim_delta'
    X = lb' .+ deltas
    z = map(i -> disjoint_poly_1d(X[i, :]...), 1:size(X, 1))
    return X, z
end

# full data
X, y = generate_disjoint1d_data(n_full_data, [0], [5], global_rng)
...
# analytical solution
@syms x_ana
expr1 = (x_ana-4)^2 * .2*x_ana^3 - 3x_ana -3
expr2 = (x_ana-4)^2 * .2*x_ana^3 - 3x_ana -13
analytical = integrate(expr1, 0, 2.5) +  integrate(expr2, 2.50000001, 5)  # -56.666

# quadrature solution
quadrature(disjoint_poly_1d, [0], [5], 0)  # -56.666
...
function run_once(;
    ...
    ############### GENERATE DATA ################
    X_train, y_train =  generate_data(generate_disjoint1d_data, n_train, lb, ub, noise_sd, rng)
    ...
    ### quadrature
    quad_est = quadrature(disjoint_poly_1d, lb, ub, 0, n_train, rng).u

    ### mc integration
    mc_est, mc_sd = mc_quadrature(disjoint_poly_1d, lb, ub, n_train, noise_sd, rng)

    # quasi mc integration
    qmc_est, qmc_sd = mc_quadrature_with_data(disjoint_poly_1d, lb, ub, X_train, noise_sd, rng)
...
CSV.write("experiments/1D/results/disjoint_1d_means.csv", mean_df)
CSV.write("experiments/1D/results/disjoint_1d_stds.csv", std_df)
CSV.write("experiments/1D/results/disjoint_1d_err_means.csv", err_df)
CSV.write("experiments/1D/results/disjoint_1d_err_stds.csv", err_σ_df)
CSV.write("experiments/1D/results/disjoint_1d_data.csv", data)
